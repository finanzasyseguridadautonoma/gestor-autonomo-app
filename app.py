...
st.markdown("""
    ...
    /* --- NUEVO: ESTILO TARJETA PARA EL LOGIN --- */
    /* Esto crea el efecto de caja blanca flotante */
    div[data-testid="stVerticalBlock"] > div[style*="background-color"] {
        /* Streamlit a veces es caprichoso, usamos un contenedor nativo abajo mejor */
    }
    
    /* BOTONES MEJORADOS */
    .stButton > button {
        border-radius: 12px; font-weight: 600; border: none;
        background-color: #2563EB; color: white; /* Azul sólido para destacar más */
...
if st.session_state['user'] is None:
    ...
    # Ajustamos columnas: [1, 2.2, 1] -> Login un poco más ancho para que respire
    ...
    with col_izq:
        ...
        st.write("") 
        st.write("")
        st.write("") # Un espacio extra para alinear con el centro de la tarjeta login
        st.markdown("""
    ...
    # --- 2. CENTRO: LOGIN (LA JOYA DE LA CORONA) ---
    with col_login:
        if st.session_state['supabase'] is None:
            st.error("❌ Error de conexión")
        else:
            # --- EL EFECTO TARJETA ---
            # Usamos un container con borde=True para simular la tarjeta
            # Streamlit lo pintará con fondo blanco/grisáceo que destaca sobre el fondo
            with st.container(border=True):
                st.write("") # Aire arriba
                ...
                with tab2:
                    ...
                    st.write("")
                    # Botón secundario con otro estilo si quisieras, pero azul está bien
    ...
    with col_der:
        ...
        st.write("") 
        st.write("") 
        st.write("") # Alineamos con el contenido del login
        ...
        with st.container(border=True):
            ...
            # --- TIPO DE BOTÓN 'PRIMARY' (ROJO POR DEFECTO EN STREAMLIT) ---
            st.link_button(
                "🎁 Cuenta Gratis", 
